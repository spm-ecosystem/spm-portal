...
- Global properties like \`author\`, \`description\`, \`targetUrl\`, and \`minEngineVersion\` present in the destination JSON file are **preserved** and merged back into the compilation output.
- This ensures GitOps metadata is maintained across compilations without manual restoration.
`,P=`**Veneer Coder** (\`spm-veneer-coder\`) is a specialized, compiler-aware LLM subagent fine-tuned specifically for generating valid **Veneer Spec (\`.vnr\`)** code, CSS design tokens, and modernizing unstructured legacy HTML into React Shadow DOM interfaces.
...
\`\`\`
`,F=`# Veneer Spec Language Reference Manual
...
* **Syntax**:
  ...
  \`\`\`
`,I=`# Veneer Spec — Extended Language Reference & Example Cookbook
...
`,L=`# Veneer Spec Language Reference Manual
...
\`\`\`
`,R=e(t(),1),z=n();function B(e){return e.replace(/\$\$\\text\{Selector\}\s*\\quad\s*\|\s*\\quad\s*\\text\{Operation\}\$\$/g,"`selector | operation`").replace(/\$\$\\text\{([^\}]+)\}\s*\\quad\s*\|\s*\\quad\s*\\text\{([^\}]+)\}\$\$/g,"`$1 | $2`").replace(/\$\$(.*?)\$\$/g,"`$1`")}function V(e){let t=B(e);return t.includes(`Component Specification`)&&(t=t.replace(/^#\s+.*?Component Specification[\s\S]*?(?:---|##\s+Overview|##\s+1\.)/i,`## Overview`)),t}function H(e){if(typeof window>`u`)return e;let t=new DOMParser().parseFromString(e,`text/html`);return t.querySelectorAll(`pre code`).forEach(e=>{let t=e.textContent||``;e.innerHTML=i(t)}),t.body.innerHTML}var U=Object.assign({"/src/docs/LayoutPrimitives.md":a,"/src/docs/UiCommentListPage.md":o,"/src/docs/UiDashboardPage.md":s,"/src/docs/UiDevDiagnosticPanel.md":c,"/src/docs/UiFormContainer.md":l,"/src/docs/UiHeroLanding.md":u,"/src/docs/UiImageCard.md":d,"/src/docs/UiImageViewer.md":f,"/src/docs/UiModernGridPage.md":p,"/src/docs/UiNavHeader.md":m,"/src/docs/UiNestedTreeTable.md":h,"/src/docs/UiPaginationBar.md":g,"/src/docs/UiPostDetails.md":_,"/src/docs/UiScrollPanel.md":v,"/src/docs/UiSearchBar.md":y,"/src/docs/UiSplitLayout.md":b,"/src/docs/UiStatsDashboard.md":x,"/src/docs/UiTable.md":S,"/src/docs/UiTableListPage.md":C,"/src/docs/UiTabs.md":w,"/src/docs/UiTagBadge.md":T,"/src/docs/UiTerminalConsole.md":E,"/src/docs/UiToastContainer.md":D,"/src/docs/cli_tooling.md":O,"/src/docs/component-specs.md":k,"/src/docs/components.md":A,"/src/docs/composition_matrix.md":j,"/src/docs/manifest-schema.md":M,"/src/docs/manifest_schema.md":N,"/src/docs/spm_veneer_coder.md":P,"/src/docs/veneer-reference.md":F,"/src/docs/veneer-spec-reference.md":I,"/src/docs/veneer_spec.md":L});function W(e){try{let t=e.split(`/`).pop()?.replace(/\.md$/,``);if(!t)return null;let n=t.replace(/Page$/,``),r=Object.keys(U).find(e=>{let r=e.split(`/`).pop()?.replace(/\.md$/,``)||``;return r.toLowerCase()===t.toLowerCase()||r.toLowerCase()===n.toLowerCase()});return r?U[r]:null}catch{return null}}function G({url:e,fallbackContent:t}){let[n,i]=(0,R.useState)(``),[a,o]=(0,R.useState)(!0),[s,c]=(0,R.useState)(null);return(0,R.useEffect)(()=>{let n=!0;o(!0),c(null);let a=W(e);return(a?Promise.resolve(a):fetch(e).then(e=>{if(!e.ok)throw Error(`HTTP error! status: ${e.status}`);return e.text()}).then(e=>{if(e.trim().startsWith(`<!doctype html>`)||e.trim().startsWith(`<html`)||e.includes(`@vite/client`))throw Error(`Documentation file not found on server.`);return e})).then(async e=>{if(!n)return;let t=V(e),a=H(await r.parse(t));i(a),o(!1)}).catch(e=>{if(n){if(t){let e=V(t);Promise.resolve(r.parse(e)).then(e=>{if(n){let t=H(e);i(t),o(!1)}})}else c(e.message),o(!1)}}),()=>{n=!1}},[e,t]),a?(0,z.jsx)(`div`,{style:{padding:`2rem 0`,color:`var(--text-muted)`,fontFamily:`var(--font-mono)`,fontSize:13},children:`Loading documentation...`}):s?(0,z.jsxs)(`div`,{style:{padding:`1.25rem`,background:`var(--bg-surface)`,border:`1px solid var(--border-contrast)`,borderRadius:6,color:`var(--text-muted)`,fontSize:13,lineHeight:1.6},children:[(0,z.jsx)(`p`,{style:{margin:`0 0 0.5rem`,color:`#ffffff`,fontWeight:600},children:`Documentation Spec`}),(0,z.jsxs)(`p`,{style:{margin:0},children:[`Technical specification for this component is synchronized directly from `,(0,z.jsx)(`code`,{children:`spm-components`}),` documentation repository.`]})]}):(0,z.jsx)(`div`,{className:`prose-spm`,dangerouslySetInnerHTML:{__html:n}})}export{G as t};